...
export default class Folder<T = any> extends React.PureComponent<{
  ...
  public render() {
    const { scope, name, getFullId } = this.props

    const fullId = scope.length ? getFullId!(scope[scope.length - 1], scope.slice(0, -1)) : ''
    return (
      <div className={prefix('folder')} ref={this.folderRef}>
        {name && (
          <div
            className={prefix('tab-input', 'full', 'file', this.isSelected(fullId) ? 'selected' : '')}
            data-file-key={fullId}
        ...
        <div className={prefix('properties')}>{this.renderProperties()}</div>
        {this.renderShadows()}
      </div>
    )
  }
  ...
  private renderProperties() {
    ...
    return keys.map((key) => {
      ...
      if (children && (isArray(children) ? children.length : isObject(children))) {
        return (
          <Folder<T>
            ...
            selected={selected}
            onSelect={isMove ? undefined : onSelect}
      ...
      return (
        <div
          key={fullId}
          className={prefix('file', this.isSelected(fullId) ? 'selected' : '')}
          data-file-key={fullId}
          onClick={isMove ? undefined : this.onClick}
        >
  ...
  private renderShadows() {
    const { FileComponent, getName, scope } = this.props
    if (scope.length) {
      return
    }
    return (
      <div className={prefix('shadows')} ref={this.shadowRef}>
        {this.state.shadows.map((info) => {
          const { scope: fileScope, value, fullId, id } = info
          const name = getName!(value, id)
          return (
            <div key={fullId} className={prefix('file', 'selected', 'shadow')}>
  ...
  private onDrag = (e: OnDrag) => {
    ...
    const { isMoveChildren, checkMove } = this.props
  ...
  private flatChildren(
    ...
    const keys = Object.keys(properties)
    const { getFullId, getId, getChildren } = this.props
  ...
  private clearPointers() {
    ;[].slice
      .call(document.querySelectorAll<HTMLElement>('.scena-top-pointer, .scena-bottom-pointer'))
